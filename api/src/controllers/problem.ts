...
async function handleListProblems(req: Request, res: Response): Promise<any> {
  const problems = await prismaClient.problem.findMany();
  return res.status(200).json({ problems });
}

async function handleCreateProblem(req: Request, res: Response): Promise<any> {
  const {
    title,
    description,
    difficulty,
  ...
  if (
    ...
    return res.status(400).json({
      error:
        "All fields (title, description, difficulty, inputFormat, outputFormat, examples) are required.",
    });
  }
